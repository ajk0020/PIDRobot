...
def main():
    ...
    while True:
        ...
        if(len(puckARRAY) == 2):
            x1 = float(puckARRAY[0][0])
            y1 = float(puckARRAY[0][1])

            x2 = float(puckARRAY[1][0])
            y2 = float(puckARRAY[1][1])
                ...
                if(reflect == True):
                    ...
                    while(reflect == True):
                        ...
                        if((x1Predict == x2Predict) and (y1Predict == y2Predict)):
                            distance = math.sqrt((x2Predict - float(puckARRAY[0][0])) ** 2 + (y2Predict - float(puckARRAY[0][1])) ** 2)
                            theta = math.acos(float(abs(x2Predict-float(puckARRAY[0][0])) / distance))*(180/math.pi)
